...
def setup_logging(
    ...
    *,
    ...
    Effects
    -------
    - Creates/gets logger `logger_name` (default: "elasnetmt")
    - Attaches a StreamHandler if not present, with formatter:
          "ELASNETMT %(levelname)s | %(message)s"
    ...
    if stream_handler is None:
        ...
        # "ELASNETMT WARNING | <Category>: <message>"
        # (For normal logs, <Category> won't appear, but the prefix still matches.)
        formatter = logging.Formatter("ELASNETMT %(levelname)s | %(message)s")
