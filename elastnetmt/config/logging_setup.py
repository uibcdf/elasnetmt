...
def setup_logging(
    ...
    *,
    ...
    logger_name: str = "elastnetmt",
) -> logging.Logger:
    """
    Configure ElastNetMT logging and (optionally) capture Python warnings.

    Effects
    -------
    - Creates/gets logger `logger_name` (default: "elastnetmt")
    - Attaches a StreamHandler if not present, with formatter:
          "ELASTNETMT %(levelname)s | %(message)s"
    ...
    if stream_handler is None:
        ...
        # "ELASTNETMT WARNING | <Category>: <message>"
        # (For normal logs, <Category> won't appear, but the prefix still matches.)
        formatter = logging.Formatter("ELASTNETMT %(levelname)s | %(message)s")
