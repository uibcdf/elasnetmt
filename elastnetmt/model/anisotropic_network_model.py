...
class AnisotropicNetworkModel(ElasticNetworkModel):
    ...
    def _solve(self):
        ...
        if self.n_nodes > 2 and np.isclose(self._eigenvalues[6], 0.0, atol=1e-8):
            smonitor.emit_from_catalog("ENM-E020", source="elastnetmt.model.AnisotropicNetworkModel")
        if np.any(self._eigenvalues[6:] < -1e-6):
            smonitor.emit_from_catalog("ENM-E030", min_ev=float(np.min(self._eigenvalues)), source="elastnetmt.model.AnisotropicNetworkModel")
