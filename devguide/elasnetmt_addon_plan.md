# ElasNetMT Add-on Plan For MolSysViewer

This document defines the development plan for the **ElasNetMT** add-on that will expose elastic-network analysis inside **MolSysViewer**.

The add-on must be developed from the **ElasNetMT** repository. `molsysviewer` remains the host application and contract provider, but the product logic, runtime state, scientific adapters, and tests for the add-on belong here.

## Implementation Status

Current state as of April 7, 2026:

- the in-tree package `molsysviewer_elasnetmt` exists and is importable,
- the add-on exposes `AddonSpec`, lifecycle hooks, runtime state, demo helpers, workbench helpers, and export helpers,
- contact links, ANM mode vectors, and anisotropy ellipsoids are rendered from real ElasNetMT models,
...
Provide a first-class interactive bridge between ElasNetMT models and MolSysViewer scenes so that a user can:
...
The first ElasNetMT add-on release should therefore stay Python-driven. It should not depend on a rich custom frontend panel or on host-side extensions that do not yet exist.

## Repository Direction

The implementation should start in this repository as a local package named `molsysviewer_elasnetmt`.

This keeps three things aligned:

- the scientific logic stays close to `GaussianNetworkModel` and `AnisotropicNetworkModel`,
- regression tests can live next to ElasNetMT physics and API tests,
...
- one workspace: `elasnetmt` with title `Elastic Networks`,
...
## Phase 1: Add-on Skeleton In ElasNetMT

*Status: IMPLEMENTED*

Create a local package `molsysviewer_elasnetmt` with:
...
- MolSysViewer can import and register `molsysviewer_elasnetmt` directly from this repository.
...
- translates ElasNetMT outputs into MolSysViewer overlay payloads.
...
- the add-on can be kept in-tree or split into `molsysviewer-elasnetmt` with minimal churn.
...
3. Decide whether `molsysviewer_elasnetmt` remains in-tree or is extracted into its own distribution.
...
- support for every ElasNetMT analysis feature before the overlay pipeline is stable.
...
2. Add end-to-end checks that the ElasNetMT add-on state survives HTML export and replay cleanly.
3. Prepare a split-ready package boundary if the add-on is going to become `molsysviewer-elasnetmt`.
