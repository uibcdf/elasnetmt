...
As ElastNetMT moves toward its first stable release (1.0.0), breaking changes are expected. To minimize friction for users, we follow the **MolSysSuite Deprecation Standard**.

## Protocol for Breaking Changes

1.  **Tagging:** Any function or method to be removed must be decorated with `elastnetmt_deprecation_warning`.
...
```python
from elastnetmt._private.warnings import warn_deprecation
